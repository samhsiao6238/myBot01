        ...
        if "message" in json_data["events"][0]:
            ...
            if (
                ...
                if text == "地震資訊" or text == "地震":  # 如果是地震相關的文字
                    ...
                    push_message(
                        msg[0], user_id, CHANNEL_ACCESS_TOKEN
                    )  # 傳送地震資訊 ( 用 push 方法，因為 reply 只能用一次 )
                    reply_image(
                        msg[1], reply_token, CHANNEL_ACCESS_TOKEN
                    )  # 傳送地震圖片 ( 用 reply 方法 )
                elif (
                    text == "雷達回波圖" or text == "雷達回波"
                ):  # 如果是雷達回波圖相關的文字
                    # 傳送雷達回波圖 ( 加上時間戳記 )
                    reply_image(
                        f"https://cwbopendata.s3.ap-northeast-1.amazonaws.com\
                        /MSC/O-A0058-001.png?\
                        {time.time_ns()}",
                        reply_token,
                        CHANNEL_ACCESS_TOKEN,
                    ...
                    # 如果是一般文字，直接回覆同樣的文字
                    reply_message(
                        text, reply_token, CHANNEL_ACCESS_TOKEN
                    )
...
def forecast(address):
    ...
    try:
        code = "你的氣象資料授權碼"
        url = f"https://opendata.cwa.gov.tw/\
            fileapi/v1/opendataapi/F-C0032-001?Authorization={code}\
            &downloadType=WEB&format=JSON"
        # 取得主要縣市預報資料
        f_data = requests.get(url)
        # json 格式化訊息內容
        f_data_json = f_data.json()
        location = f_data_json["cwbopendata"]["dataset"][
            "location"
        ]  # 取得縣市的預報內容
        for i in location:
            ...
            ci8 = i["weatherElement"][2]["time"][0]["parameter"][
                "parameterName"
            ]  # 舒適度
        ...
        for i in area_list:
            if i in address:  # 如果使用者的地址包含縣市名稱
                ...
                # 將進一步的預報網址換成對應的預報網址
                url = f"https://opendata.cwa.gov.tw/api/v1/\
                    rest/datastore/{json_api[i]}?Authorization={code}\
                        &elementName=WeatherDescription"
                f_data = requests.get(url)  # 取得主要縣市裡各個區域鄉鎮的氣象預報
                f_data_json = f_data.json()  # json 格式化訊息內容
                location = f_data_json["records"]["locations"][0][
                    "location"
                ]  # 取得預報內容
                break
        for i in location:
            city = i["locationName"]  # 取得縣市名稱
            wd = i["weatherElement"][0]["time"][1]["elementValue"][0][
                "value"
            ]  # 綜合描述
...
def current_weather(address):
    ...
    def get_data(url):
        ...
        for i in location:
            ...
            humd = check_data(
                round(float(
                    i["weatherElement"][4]["elementValue"]["value"]
                ) * 100, 1)
            )
            r24 = check_data(
                i["weatherElement"][6]["elementValue"]["value"]
            )  # 累積雨量
            ...
            city_list[city]["temp"].append(
                temp
            )  # 記錄主要縣市裡鄉鎮區域的溫度 ( 串列格式 )
            city_list[city]["humd"].append(
                humd
            )  # 記錄主要縣市裡鄉鎮區域的濕度 ( 串列格式 )
            city_list[city]["r24"].append(
                r24
            )  # 記錄主要縣市裡鄉鎮區域的雨量 ( 串列格式 )
    ...
    def msg_content(loc, msg):
        ...
        for i in loc:
            if i in address:  # 如果地址裡存在 key 的名稱
                if loc[i]["temp"] is not False:
                    temp = f"氣溫 {loc[i]['temp']} 度，"
                else:
                    temp = ""
                if loc[i]["humd"] is not False:
                    humd = f"相對濕度 {loc[i]['humd']}%，"
                else:
                    humd = ""
                if loc[i]["r24"] is not False:
                    r24 = f"累積雨量 {loc[i]['r24']}mm"
                else:
                    r24 = ""
    ...
    try:
        ...
        code = "你的氣象資料授權碼"
        get_data(
            f"https://opendata.cwa.gov.tw/fileapi/v1/\
                opendataapi/O-A0001-001?Authorization=\
                    {code}&downloadType=WEB&format=JSON"
        )
        get_data(
            f"https://opendata.cwa.gov.tw/fileapi/v1/\
                opendataapi/O-A0003-001?Authorization=\
                    {code}&downloadType=WEB&format=JSON"
        )
...
def aqi(address):
    ...
    try:
        ...
        url = "https://data.epa.gov.tw/api/v2/\
            aqx_p_432?api_key=e8dd42e6-9b8b-43f8-991e-b3dee723a52d&limit=\
                1000&sort=ImportDate%20desc&format=JSON"
        ...
        for i in a_data_json["records"]:  # 依序取出 records 內容的每個項目
            ...
            city_list[city].append(
                aqi
            )  # 將各個縣市裡的鄉鎮區域空氣 aqi 數值，以串列方式放入縣市名稱的變數裡
        for i in city_list:
...
def earth_quake():
    ...
    try:
        code = "你的氣象資料授權碼"
        url = f"https://opendata.cwa.gov.tw/api/v1/\
            rest/datastore/E-A0016-001?Authorization={code}"
...
def push_image(msg, uid, token):
    ...
    body = {
        ...
        "messages": [{
            "type": "image",
            "originalContentUrl": msg, "previewImageUrl": msg
        }],
    }
    req = requests.request(
        "POST",
